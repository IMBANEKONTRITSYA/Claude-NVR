...
**Масштаб сценария — выбранный, а не предписанный.** Действующая редакция
ТЗ числа камер не задаёт и прямо запрещает его хардкодить (§22): §1
объявляет диапазон 12–250+ и аналитику 0..N, всё конфигурируемо. По §20
объект такого размера — «Большой (128-256+ камер)» на 2× Xeon, а 120 камер
записи и 2 аналитики берутся отсюда же как **представительная точка**
внутри этого класса; то же число служит примером в §17 («камеры 1-60 на
node 0, 61-120 на node 1 — пример для 120 камер»). Оба значения задаются
переменными окружения (`LOCUST_FW_CAMERAS`, `LOCUST_FW_ANALYTICS_CAMERAS`)
и меняются под объект без правки кода.

Раньше здесь стояла ссылка на «§16, который требует пересмотра сценария
под масштаб §26, а §26 задаёт его буквально: 120 камер». Это цитата
**удалённой редакции ТЗ**: в действующей §16 — «Планирование ресурсов»,
§26 — «Стратегия развёртывания», и чисел камер там нет. Сам масштаб
сценария от этого не меняется — меняется его обоснование, которое до сих
пор ссылалось на несуществующее требование. (До цикла 27 файл реализовывал
масштаб ещё более старой редакции — 16 камер, все в аналитике — и
ссылался на «SPEC.md §14».)
...
своей страницы живой сетки (§4 — грид до 16 на экран, §3 — выбор
...
Для полноценной проверки объекта целиком (120 потоков записи + 2 аналитики, 24 ч)
...
# Умолчание, а не требование ТЗ: §1 объявляет 12–250+ камер, §22 запрещает
# хардкодить их число, а §20 относит объект такого размера к классу
# «Большой (128-256+ камер)». 120 — представительная точка внутри класса и
# то же число, которым §17 иллюстрирует раскладку по NUMA-нодам. Переменной
# окружения меняется под объект. До цикла 27 здесь стояло 16 — число ещё
# более старой редакции, где вся система была на 16 камер.
TARGET_CAMERAS = int(os.environ.get("LOCUST_FW_CAMERAS", "120"))
# SPEC §2: аналитика — «только для выбранных камер», их число
# конфигурируемо и по §16 упирается в ресурсы сервера. Два — консервативная
# точка для CPU-развёртывания без GPU; остальные заводятся в record_only
# (режим по умолчанию по §2), и именно такое соотношение нагружает API так,
# как боевой объект.
ANALYTICS_CAMERAS = int(os.environ.get("LOCUST_FW_ANALYTICS_CAMERAS", "2"))
# SPEC §4: «Мозаичный режим (грид): 1, 4, 9, 16 камер на экран»; §3 —
# «группировка/пагинация для больших объектов (100+ камер)». Один оператор
# смотрит страницу, а не весь парк сразу.
GRID_PAGE_SIZE = 16
...
def setup_cameras(environment, **kwargs):
    ...
    нуля, а сценарий — про парк заданного размера, а не про "сколько уже есть".

    Режимы расставляются по ТЗ: первые ANALYTICS_CAMERAS — `analytics`,
    остальные — `record_only` (§2, §3). Это не косметика: аналитические
...
class CameraViewerUser(HttpUser):
    ...
    По §4 сетка — «грид: 1, 4, 9, 16 камер на экран», а по §3 — «выбор
    ...
    def record_layer_status(self):
        """Карточка «Слой записи» — статус всех 120 потоков (§9).
    ...
    def storage_report(self):
        """Прогноз места (§5, §16): SUM(size_bytes) по всему архиву."""
