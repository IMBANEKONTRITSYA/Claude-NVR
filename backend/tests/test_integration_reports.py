...
def _cleanup_seeded(pg_conn):
    """Строки, засеянные `_insert_camera_person_event`, убираются после
    каждого теста. До цикла 24 они оставались в БД: в CI это незаметно
    (свежий контейнер на прогон), но локально камеры копились от прогона к
    прогону, а с появлением предела на камеры в режиме analytics (SPEC §1)
    накопленные камеры начали ронять соседние тесты."""
...
def _insert_camera_person_event(pg_conn, _seeded=None):
    with pg_conn.cursor() as cur:
        cur.execute(
...
def test_appearances_csv_contains_seeded_row(client, admin_token, pg_conn, _cleanup_seeded):
    _insert_camera_person_event(pg_conn, _cleanup_seeded)
...
def test_appearances_xlsx_downloads_with_correct_content_type(client, admin_token, pg_conn, _cleanup_seeded):
    _insert_camera_person_event(pg_conn, _cleanup_seeded)
...
def test_persons_summary_csv(client, admin_token, pg_conn, _cleanup_seeded):
    _insert_camera_person_event(pg_conn, _cleanup_seeded)
...
def test_persons_summary_xlsx(client, admin_token, pg_conn, _cleanup_seeded):
    _insert_camera_person_event(pg_conn, _cleanup_seeded)
...
def test_cameras_activity_csv(client, admin_token, pg_conn, _cleanup_seeded):
    _insert_camera_person_event(pg_conn, _cleanup_seeded)
...
def test_cameras_activity_xlsx(client, admin_token, pg_conn, _cleanup_seeded):
    _insert_camera_person_event(pg_conn, _cleanup_seeded)
