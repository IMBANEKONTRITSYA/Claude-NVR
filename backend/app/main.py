...
from .routers import backups as r_backups
...
# запуск», §18 строка «Управление бэкапами». Отдельный роутер, а не часть
# system: все четыре роута admin-only, тогда как большинство системных
# доступно ещё и оператору.
app.include_router(r_backups.router)
...
# Цикл 51: snapshots/avatars закрыты наблюдателю. Прежде они были открыты
# всем ролям «осознанно» — на посылке, что на них построены Стена и
# дашборд, «разрешённые наблюдателю той же матрицей». Посылка неверна:
# строки «Стена» в §18 нет вовсе, а §9 перечисляет содержимое дашборда
# поимённо («количество событий, активность по часам, топ камер») и персон
# среди него не значится. Ни одна строка §18, относящаяся к модулю §15
# («Поиск по фото», «Карточки персон», «Ручной апскейл лица»), наблюдателю
# не разрешена — а хранимый кадр лица есть содержимое карточки.
# Живой просмотр §4 от этого не страдает: рамки с подписями приходят по
# `/ws/faces` поверх видео и хранимых снимков не используют.
MEDIA_KIND_ROLES: dict[str, tuple[str, ...]] = {
    "snapshots": ("admin", "operator"),
    "avatars": ("admin", "operator"),
