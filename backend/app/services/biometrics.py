...
* осиротевший JPEG самоисправляется, и до того недостижим по API — на него
  не ссылается ни одна строка. Убирают его два разных механизма, и это
  стоит различать: `snapshots/` чистит `worker.prune_media` **по возрасту
  файла**, то есть остаток уходит в пределах срока хранения; `avatars/`
  уборка по возрасту не трогает намеренно (аватар живёт столько же, сколько
  карточка, сторож — `worker/tests/test_fileage.py::test_prune_media_keeps_avatars`),
  и осиротевшие аватары убирает `worker.prune_orphan_media()` — сверкой с
  `persons.avatar_path`, а не по возрасту.

  До цикла 53 здесь стояло «`snapshots/` и `avatars/` чистит `prune_media`
  по возрасту файла». Про `avatars/` это было неправдой: такого каталога в
  `prune_media` нет вовсе, и осиротевший аватар не убирал никто.
"""
import logging
import os
