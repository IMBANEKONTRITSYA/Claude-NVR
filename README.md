...
`Аналитика` (по умолчанию 2), а не все камеры записи. Числа камер система
нигде не фиксирует: §1 задаёт диапазон 12–250+, а §22 прямо запрещает
хардкодить количество — пределы считаются по формулам §16 от ресурсов
конкретного сервера («Автоконфигурация» в «Мониторинге»).
