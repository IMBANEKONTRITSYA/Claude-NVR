"""Отправка почты по SMTP для слоя аналитики (SPEC §6 «Алерты при детекции
(Telegram, email, звук)»).

Копия `backend/app/services/mailer.py`: у воркера нет зависимости от пакета
`app`, как и для моделей таблиц и расшифровки секретов (см. `worker.py`).
Модуль только на stdlib, поэтому копия ничего не тянет в requirements.txt.
Расхождение с оригиналом ловит `backend/tests/test_mailer_parity.py` —
сверяются TLS_MODES, SMTP_TIMEOUT_SEC и тела всех функций.

=== ОБЩАЯ ЧАСТЬ (сверяется с backend/app/services/mailer.py по AST) ===
"""
import smtplib
import ssl
