...
STAGE_BUDGETS_SEC: dict[str, float] = {
    ...
    "motion_prune": 900.0,
    # Этапа `disk_quota` здесь больше нет — по тому же доводу, по которому
    # выше ушёл `cleanup`, а в цикле 55 `model_load`.
    #
    # Он стоял тут с бюджетом 900 секунд, страницей ниже объяснения, почему
    # такой строки в этой таблице быть не должно, и означал то же самое:
    # пятнадцать минут без синхронизации путей, без индексации сегментов и
    # без публикации статусов потоков — считаются нормой.
    #
    # Разница с уборкой — в пользу выноса, а не против. Уборка идёт раз в
    # час, перезапись — **каждым проходом менеджера (~10 с)**, и ровно
    # потому, что «между часовыми проходами 120 камер успевают дописать
    # ~108 ГБ». Пока том переполнен, долгий проход не эпизод, а
    # установившийся режим. Довод «у неё `LIMIT 5000`, предел на объём
    # работы есть» ограничивает строки, а не время: 5000 сегментов — до
    # 10 000 `unlink()` по HDD, тот же порядок, под который и был выставлен
    # бюджет в 900 с.
    #
    # Сторожить нить перезаписи этот сторож не должен по тому же доводу,
    # что нити уборки и загрузки модели: его реакция — убийство процесса, а
    # перезапуск воркера из-за медленного тома остановил бы заодно слой
    # записи, то есть нарушил §2 самим лечением. Отказ перезаписи виден
    # иначе, и у него, в отличие от уборки, есть собственный алертинг:
    # `check_disk_alerts()` остаётся в нити менеджера и шлёт «переполнение
    # диска» в Telegram и на почту, а состояние прохода едет в §9
    # (`QUOTA_STATUS`, см. `start_quota_pass`).
    "disk_alerts": 120.0,
