...
STAGE_BUDGETS_SEC: dict[str, float] = {
    ...
    "record_status": 120.0,
    # Этапа `cleanup` здесь больше нет — по той же причине, по которой в
    # цикле 55 отсюда ушёл `model_load`, и с той же ценой прежней записи.
    #
    # Он стоял тут с бюджетом 900 секунд и комментарием «на объекте это
    # десятки тысяч unlink() по HDD, и медленно здесь — норма, а не сбой».
    # Норма-то норма, но пока уборка шла в нити менеджера, число означало
    # вот что: пятнадцать минут, в течение которых не публикуются статусы
    # потоков (§9: алерт «потеря потока» опаздывает, ключ `record:layer` с
    # TTL 120 с протухает), не выполняется циклическая перезапись и не
    # индексируются сегменты, — считаются нормой. §2 запрещает ровно это,
    # поэтому с цикла 57 уборка ушла в свою нить (`start_cleanup_pass`), а
    # менеджер больше не отмечается этим этапом ни разу.
    #
    # Сторожить нить уборки этот сторож не должен по тому же доводу, что и
    # нить загрузки модели: его реакция — убийство процесса, а перезапуск
    # воркера из-за медленной уборки архива остановил бы заодно и слой
    # записи. Затянувшийся проход виден предупреждением в журнале: пока он
    # идёт, следующий не стартует и пишет об этом.
    "motion_prune": 900.0,
    "disk_quota": 900.0,
    "disk_alerts": 120.0,
