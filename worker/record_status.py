...
# --- алерты §9 ------------------------------------------------------------
#
# Здесь, а не в worker.py, по той же причине, по которой `onvif_api.py`
# вынесен из `embed_api.py`: worker.py тянет cv2/insightface/onnxruntime, и
# всё, что живёт в нём, в лёгкой CI-джобе воркера **пропускается**. Алерты,
# написанные там и проверенные только в песочнице, повторили бы дефект,
# который этот код закрывает: выглядит рабочим, а в CI не выполняется ни
# разу. Отправка (Telegram/SMTP) и кулдаун через Redis остаются в worker.py
# — им нужны сеть и соединение; сюда вынесено решение, а не действие.

# Ярлыки видов алертов: (что случилось, тема письма). Ключ идёт и в кулдаун
# Redis, и в текст, поэтому он один на оба, а не два похожих литерала.
RECORD_ALERTS = {
    "stream_lost": ("потерян поток записи", "Потеряны потоки записи"),
    "segment_missing": ("пропуск записи сегмента", "Запись не идёт"),
}

# Сколько номеров камер показывать в теле. Остальные — счётчиком: письмо со
# ста двадцатью номерами нечитаемо, а Telegram обрежет его по своему пределу
# длины, причём молча и посередине.
ALERT_IDS_SHOWN = 20


def alert_batch(kind: str, camera_ids, due) -> tuple[list[int], str, str] | None:
    """Одно сообщение на пачку камер. `None`, если сообщать не о чем.

    Args:
        kind: ключ из `RECORD_ALERTS`.
        camera_ids: камеры, по которым сработало событие в этом проходе.
        due: предикат «кулдаун по этой камере истёк». Инъекцией, а не
            обращением к Redis: решение о том, что и как сказать, проверяемо
            без сети, а всё сетевое остаётся у вызывающего.

    Returns:
        `(камеры, тема, текст)` либо `None`.

    Одно сообщение, а не по одному на камеру: моргнувший коммутатор роняет
    весь объект в одном проходе, и сто двадцать писем подряд — это ровно то,
    после чего администратор выключает уведомления. Следующий алерт, уже
    настоящий, не прочитает никто.
    """
    fresh = [cid for cid in camera_ids if due(cid)]
    if not fresh:
        return None
    what, subject = RECORD_ALERTS[kind]
    shown = ", ".join(f"#{cid}" for cid in fresh[:ALERT_IDS_SHOWN])
    if len(fresh) > ALERT_IDS_SHOWN:
        shown += f" и ещё {len(fresh) - ALERT_IDS_SHOWN}"
    text = (f"⚠️ FaceWatch: {what} — камер {len(fresh)}: {shown}. "
            f"Архив по ним не пополняется.")
    return fresh, subject, text
