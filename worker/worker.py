...
def _drop_segments(s, segments) -> int:
    ...
    **Строки сносятся одним `DELETE ... WHERE id IN (...)`, а счётчик берётся
    из `rowcount`, а не из длины списка.** Это единственное, что делает путь
    терпимым к параллельному удалению того же сегмента. Три прохода уборки —
    retention (`cleanup_old`), циклическая перезапись (`enforce_disk_quota`)
    и режим «запись только при движении» (`prune_motionless_segments`) —
    выбирают кандидатов независимо и по пересекающимся признакам: старейший
    сегмент переполненного диска обычно и просрочен тоже. Пока все три шли в
    одной нити менеджера, пересечься они не могли, и `session.delete()` по
    ORM-объекту был безопасен. Как только уборка ушла в свою нить (§2:
    долгий этап не должен задерживать соседние), совпадение выборок стало
    штатным.

    **Чем это грозило на самом деле — не тем, чего ждали.** Разбор цикла 55
    предсказывал `StaleDataError` на flush'е и обрыв всего прохода. Проверено
    на настоящем Postgres 16 и на SQLite (`test_retention_rotation.py`,
    раздел про параллельные проходы): `session.delete()` по уже снесённой
    строке **не падает** — SQLAlchemy ограничивается `SAWarning`. Ошибку она
    поднимает только там, где у маппера настроен `version_id_col`; в её
    собственном исходнике (`orm/persistence.py`) на этом месте стоит TODO
    «почему DELETE только предупреждает, а UPDATE бросает». Обрыва уборки,
    из-за опасности которого правку отложили на цикл, не было бы.

    **Что портилось в действительности — счёт.** Опоздавший проход
    засчитывал себе чужое удаление: длина списка не знает, сколько строк
    ушло. На этих числах стоят логи ротации и отчёт §9 о снятом объёме, а
    при пересечении retention с циклической перезаписью один сегмент
    попадал в оба счёта. `rowcount` отвечает на вопрос честно: удаливший
    строку получает 1, опоздавший — 0.

    Файл и миниатюру опоздавший удалит повторно, и это безвредно:
    `os.path.exists` и `drop_thumb` спокойно переживают отсутствие файла.
    """
    ...
    # Пачками, а не одним списком на весь проход: `enforce_disk_quota`
    # выбирает до 5000 строк, и столько же параметров в одном `IN` — это
    # запрос, который Postgres разбирает дольше, чем выполняет.
    dropped = 0
    for start in range(0, len(ids), 1000):
        chunk = ids[start:start + 1000]
        # synchronize_session=False: сверять identity map незачем, объекты
        # тут же отцепляются от сессии ниже.
        dropped += s.execute(
            delete(VideoSegment).where(VideoSegment.id.in_(chunk)),
            execution_options={"synchronize_session": False},
        ).rowcount
    # Объекты отцепляются от сессии: иначе commit пометил бы их
    # просроченными, и любое последующее обращение к полю ушло бы за строкой,
    # которой уже нет (`ObjectDeletedError`).
    for seg in segments:
        if seg in s:
            s.expunge(seg)
    return dropped
...
# Нить последнего запущенного прохода уборки архива. Модульная переменная, а
# не локальная в `manager()`: по ней проверяется, не идёт ли проход до сих
...
# Нить последнего прохода циклической перезаписи и его состояние. Заведены по
# тому же поводу и по тому же образцу, что пара выше, — см. `start_quota_pass`.
_quota_thread: threading.Thread | None = None
QUOTA_STATUS = CleanupStatus()


def _cleanup_bg():
    """Проход уборки архива целиком: retention + осиротевшие файлы.

    Приоритет здесь НЕ понижается, в отличие от `_recluster_bg`. Понижение
    имеет смысл против конкуренции за процессор (DBSCAN считает), а этот
    проход — почти чистый ввод-вывод: десятки тысяч `unlink()` и обход
    каталогов. `nice` на диск не влияет, зато отложил бы освобождение места
    на переполненном томе, ради которого проход и запускается.

    Два `try`, как было в менеджере: сиротство после удаления камеры не
...
def start_cleanup_pass() -> bool:
    """Запускает проход уборки архива в своей нити (SPEC §2, §5, §9).

    **Почему не в нити менеджера, как было до этого цикла.** `cleanup_old()`
    — единственный этап прохода без предела на объём работы: у циклической
    перезаписи `LIMIT 5000`, у уборки по движению `LIMIT 1000`, а retention
    берёт всё, что просрочено, и делает по два `unlink()` на сегмент. На
    120 камерах суточная порция — десятки тысяч файлов, и бюджет этапа в
    сторожe живости стоял соответствующий: 900 секунд.

    Пока проход шёл в нити менеджера, эти 900 секунд означали, что
    **четверть часа не выполняется управляющий контур слоя записи**:

    * `publish_record_layer_status` — значит алерт «потеря потока» (§9)
      опаздывает на всё это время, а ключ `record:layer` в Redis (TTL 120 с)
      успевает протухнуть, и страница мониторинга показывает пустоту;
    * `enforce_disk_quota` — циклическая перезапись. Отдельно неприятно
      именно здесь: у самого `enforce_disk_quota` стоит комментарий, почему
      он зовётся каждые ~10 с, а не раз в час, — «между часовыми проходами
      120 камер успевают дописать ~108 ГБ». Долгая уборка возвращала ровно
      тот интервал, против которого это написано;
    * `record_layer_sync` и `index_record_segments` — новые камеры не
      появляются в MediaMTX, дописанные сегменты не попадают в архив.

    §2 запрещает ровно это: «слои не имеют общих узких мест». Класс тот же,
    что у загрузки модели (цикл 55), но здесь оба конца — внутри слоя
    записи: долгий этап задерживал соседние по циклу.

    **Сторож живости этот проход намеренно не сторожит.** Его реакция —
    убийство процесса, а убивать воркер из-за медленной уборки архива
    значило бы перезапускать вместе с ней слой записи, то есть нарушать §2
    самим лечением. Зависший том виден иначе: проход не заканчивается,
    следующий не стартует, и об этом печатается предупреждение ниже.

    Возвращает False, если предыдущий проход ещё идёт.
    """
    global _cleanup_thread
    if _cleanup_thread is not None and _cleanup_thread.is_alive():
        # Проход, не уложившийся в час, — сам по себе диагноз: либо том
...
def _quota_bg():
    """Проход циклической перезаписи целиком (SPEC §5).

    Один этап, а не два, как у уборки: `enforce_disk_quota()` — сам по себе
    аварийный механизм и ни с чем не парный. Имя упавшего этапа всё равно
    копится списком — читатель состояния (§9) один и тот же, и разный вид
    поля у двух проходов пришлось бы разбирать на два случая во фронтенде.
    """
    QUOTA_STATUS.pass_started()
    failed = []
    try:
        enforce_disk_quota()
    except Exception:
        failed.append("enforce_disk_quota")
        logger.error("ошибка циклической перезаписи", exc_info=True)
    QUOTA_STATUS.pass_finished(failed)


def start_quota_pass() -> bool:
    """Запускает проход циклической перезаписи в своей нити (SPEC §2, §5, §9).

    **Почему не в нити менеджера, как было до этого цикла.** Довод здесь
    ровно тот, которым цикл 55 вынес загрузку модели, а цикл 57 — уборку
    архива, и `liveness.py` его же и формулирует: этап с бюджетом 900
    секунд в нити менеджера означает четверть часа, в течение которых не
    синхронизируются пути в MediaMTX, не индексируются сегменты и не
    публикуются статусы потоков (ключ `record:layer` с TTL 120 с успевает
    протухнуть). §2 запрещает ровно это. Строка `"disk_quota": 900.0`
    пережила обе правки, стоя в той же таблице бюджетов страницей ниже
    объяснения, почему такой строки там быть не должно.

    **Здесь этот довод весит больше, а не меньше, чем у уборки.** Три
    отличия, и все три — не в пользу прежнего порядка:

    * **Частота.** Уборка по retention идёт раз в час, и её простой — часовая
      редкость. Перезапись зовётся **каждым проходом менеджера (~10 с)** и
      ровно поэтому: «между часовыми проходами 120 камер успевают дописать
      ~108 ГБ» (комментарий при вызове). Пока том переполнен, долгий проход
      не эпизод, а установившийся режим: следующий стартует сразу за
      предыдущим, и управляющий контур записи не выполняется вовсе.
    * **Совпадение с худшим моментом.** Перезапись срабатывает тогда и
      только тогда, когда места уже нет, — то есть именно в тот час, когда
      статусы потоков и алерт «переполнение диска» (§9) нужнее всего.
    * **`LIMIT 5000` ограничивает строки, а не время.** Довод «предел на
      объём работы есть» верен буквально и не отвечает на вопрос о
      длительности: 5000 сегментов — это до 10 000 `unlink()` (файл плюс
      миниатюра) по HDD-массиву, тот же порядок, что «десятки тысяч» у
      retention. Бюджет 900 с и был выставлен под это.

    Сверх того, прежний порядок давал сторожу право убить воркер за
    медленный том: превышение 900 с — это `os._exit()` и перезапуск
    процесса вместе со слоем записи, то есть нарушение §2 самим лечением.
    Поэтому этап убран и из таблицы бюджетов (`liveness.py`).

    **Кто теперь смотрит.** Состояние прохода уезжает в §9 через
    `QUOTA_STATUS` — тем же путём, что состояние уборки и загрузки модели.
    В отличие от уборки, у отказа перезаписи есть и второй, независимый
    наблюдатель: `check_disk_alerts()` остаётся в нити менеджера и шлёт
    алерт по заполнению диска в Telegram и на почту. Он и есть тот порог,
    при котором система поднимает тревогу сама, — а состояние в §9
    отвечает на следующий вопрос дежурного: «место не освобождается —
    перезапись сломалась или просто не успевает?».

    Возвращает False, если предыдущий проход ещё идёт.
    """
    global _quota_thread
    if _quota_thread is not None and _quota_thread.is_alive():
        # Пропуск здесь — НЕ признак беды, в отличие от такого же пропуска у
        # часовой уборки, и потому не логируется предупреждением. Проход
        # запрашивается каждые ~10 с; любой, кто идёт дольше одного прохода
        # менеджера, даёт пропуск на следующем — на переполненном томе это
        # установившийся режим, а не сбой. Счётчик всё равно ведётся: он
        # отвечает на вопрос «сколько перезапись уже не поспевает за
        # запросами», но тревогу по нему не поднимают (см. §9-правило показа).
        QUOTA_STATUS.pass_skipped()
        return False
    _quota_thread = threading.Thread(target=_quota_bg,
                                     name="disk-quota", daemon=True)
    _quota_thread.start()
    return True
...
def publish_record_layer_status(cam_names) -> dict:
    ...
    payload = {"streams": list(states.values()), "summary": summarize(states),
               ...
               # Состояние циклической перезаписи (§5, §9) — по тому же
               # поводу, что и состояние уборки рядом: с этого цикла проход
               # идёт в своей нити и сторожем живости не проверяется.
               # Читается иначе, чем `cleanup`: пропуски здесь штатны
               # (проход запрашивается каждые ~10 с), а тревожен отказ —
               # сломанная перезапись означает, что запись встанет, когда
               # место кончится (см. `start_quota_pass`).
               "quota": QUOTA_STATUS.snapshot()}
    try:
...
def manager():
    ...
    while not shutdown_event.is_set():
        try:
            ...
            now = time.time()
            # Уборка архива — в своей нити, менеджер её не ждёт: обоснование
            # и цена прежнего поведения — в докстринге `start_cleanup_pass`.
            # Отметки этапа здесь больше нет намеренно; `cleanup` убран и из
            # таблицы бюджетов сторожа (liveness.py).
            ...
            # Циклическая перезапись — в своей нити, менеджер её не ждёт:
            # обоснование и цена прежнего поведения — в докстринге
            # `start_quota_pass`. Отметки этапа здесь больше нет намеренно;
            # `disk_quota` убран и из таблицы бюджетов сторожа (liveness.py).
            #
            # Запрос по-прежнему уходит каждым проходом (~10 с) — интервал не
            # изменился, изменилось только то, что менеджер не стоит на месте
            # всё время прохода. Второй поверх идущего не стартует.
            start_quota_pass()
